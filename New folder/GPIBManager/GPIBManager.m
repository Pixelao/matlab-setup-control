...
% lab equipment.
    ...
    methods (Access = public)
        ...
        function data = ReadKeithleySingle(obj,number,mode,~)
            if strcmp(mode,'VOLT')
                data = [];
                handle = obj.equipment.keithley(number);
                fprintf(handle, ':SENS:FUNC "VOLT"');
                data(end+1) = str2num(query(handle, ':SENS:DATA?'));
            
            elseif strcmp(mode,'CURR')
                data = [];
                handle = obj.equipment.keithley(num);
                fprintf(handle, ':SENS:FUNC "CURR"');
        ...
        function data=readSource(obj,numSource,mode,~)
             if (strcmp(mode,'CURR'))
       ...
       function current = ReadKeithleysCurrent(obj)
            current = [];
            for h = obj.equipment.keithley
...
% 28-2-2018: added this function to GPIB manager
        ...
        % Does something IV related
            ...
            function voltage=SourceStep(obj,numSource,mode,volt)
               if (strcmp(mode,'CURR'))
                   I = volt;
                     % Get the handle for this device
                    handle = obj.equipment.vsource(numSource); 
             ...
             % Execute a single step in a voltage ramp
            function current=SourceCurrentStep(obj,mode,numSource,I)
        ...
        % TODO: FIND WAY TO HANDLE BOTH X AND Y
        function xr = LockinReadSingle(obj,num,mode,~)
            xr = [];
            yt = [];
            % Mode should be either xy or rt
            if ~(strcmp(mode,'x') || strcmp(mode,'r') || strcmp(mode,'CH'))
                error('GPIBManager.m: Mode should be either "xy" or "rt".')
                return
            end
        ...
        function yt = LockinReadSingleY(obj,num,mode,~)
            xr = [];
            yt = [];
            % Mode should be either xy or rt
            if ~(strcmp(mode,'y') || strcmp(mode,'t') || strcmp(mode,'CH'))
                error('GPIBManager.m: Mode should be either "xy" or "rt".')
                return
            end
                h = obj.equipment.lockin(num);
        ...
        function value = SetLockin(obj,mode,num,input)
            if strcmp(mode,'VOLT')
                if ~isnumeric(num)
                    error('Input voltage type is not a number');
                end
                handle = obj.equipment.lockin(input);
                fprintf(handle,['SLVL ' num2str(freq)]);
                value = query(h, 'SLVL?');
                %disp(['Frequency set to' value])
            elseif strcmp(mode,'FREQUENCY')
                ...
                handle = obj.equipment.lockin(num);
                mode = query(handle, 'FMOD?'); % Check the reference signal mode on the lockin
                disp(['Lockin set to ' mode]);
                for p=1:length(freq)
                    fprintf(handle,['FREQ ' num2str(input)]); % Set the frequency on the lockin
                end
                value = query(h, 'FREQ?');
            end
        end
        ...
        % Magnet ramp function for measurement_class
        function data = setMagnet(obj, num, quantity_mode, input_val)
            
            curr_set = input_val;
            stepsize = 0.1;
            pausetime = 0.1;
            if (strcmp(quantity_mode,'CURR'))
                data = str2num(obj.RampMagnet(curr_set,stepsize,pausetime));
            else
                disp('Mode not supported')
            end
        end
        
        function data = readMagnet(obj, num, quantity_mode, input_val)
            
            curr_set = input_val;
            stepsize = 0.1;
            pausetime = 0.1;
            if (strcmp(quantity_mode,'CURR'))
                data = str2num(obj.ReadPSU('curr'));
            else
                disp('Mode not supported')
            end
        end
        ...
        % Ramp the current of the magnet
         function magnet = RampMagnet(obj,curr_set,stepsize,pausetime,varargin)
            ...
            bop_status = str2num(query(handle,'*ESR?:;STAT:QUES?'));
            ...
            elseif curr_now <= curr_set
                ...
                warning(['The power supply is already set to that value. Measured value: ' num2str(curr_now)])
            end
            % Actually ramp up the power supply
            fprintf(handle,'VOLT 20'); % set voltage value
            for i=1:length(curr)
         ...
         function magnet = ChangeMagnetVoltage(obj,voltage,stepsize,pausetime,varargin)
            ...
            % Actually ramp up the power supply
            fprintf(handle, 'CURR 20'); % set voltage value
            for i=1:length(volt)
